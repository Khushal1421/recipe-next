...
export default async function MealDetails({
  ...
  params: { id: string }; // id is a string because route params come as strings
}) {
  const data: Recipe = await GetRecipeById(Number(params.id)); // Convert to number
  ...
  return (
    <>
      <div className="m-10">
        ...
        <div className="mt-15">{data.summary}</div>

        <div className="mt-20">
          <h3 className="font-sans font-medium">Sources</h3>
          <div className="mt-8 flex flex-col gap-5">
            <div>{data.sourceUrl}</div>
            <div>{data.spoonacularSourceUrl}</div>
          </div>
        </div>
      </div>
    </>
